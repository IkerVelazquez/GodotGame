...
func start_timing_minigame():
	print("⏰ Iniciando minijuego de timing...")
...
func dash_to_enemy() -> void:
	...
	is_dashing = true
	$player_animations.play("Speical_pose")
	$Grunting.play()
	$Charge.play()
	...
	# Reproducir todos los efectos a la vez (o en secuencia muy rápida)
	tween.tween_callback(_play_all_special_effects)
	tween.tween_interval(0.8)  # Tiempo para mostrar efectos
	
	tween.tween_callback(Callable(self, "_hide_effects"))
	tween.tween_callback(Callable(self, "_flip_back"))
	tween.tween_property(self, "global_position", original_position, return_duration)\
...
func _play_all_special_effects():
	_play_effect_sequence()

func _play_effect_sequence():
	# Primer conjunto de efectos
	_play_single_effect(0.0, 1)
	get_tree().create_timer(0.09).timeout.connect(_play_single_effect.bind(0.09, 2))
	get_tree().create_timer(0.18).timeout.connect(_play_single_effect.bind(0.18, 3))
	get_tree().create_timer(0.27).timeout.connect(_play_single_effect.bind(0.27, 4))
	
	# Segundo conjunto de efectos
	get_tree().create_timer(0.36).timeout.connect(_play_single_effect.bind(0.36, 1))
	get_tree().create_timer(0.45).timeout.connect(_play_single_effect.bind(0.45, 2))
	get_tree().create_timer(0.54).timeout.connect(_play_single_effect.bind(0.54, 3))

func _play_single_effect(delay: float, effect_number: int):
	match effect_number:
		1:
			$SPECIAL_EFFECTS/Special_effect.play("Hit")
			$Katana_slide.play()
		2:
			$SPECIAL_EFFECTS/Special_effect2.play("Hit")
			$Katana_slide2.play()
		3:
			$SPECIAL_EFFECTS/Special_effect3.play("Hit")
			$Katana_slide3.play()
		4:
			$SPECIAL_EFFECTS/Special_effect4.play("Hit")
			$Katana_slide4.play()
...
func _hide_effects():
	...
	$Power_down.play()
	$Kanji.visible = true
	$Kanji/AnimationPlayer.play("disapear")
